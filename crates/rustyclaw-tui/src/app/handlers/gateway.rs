...
impl App {
    pub async fn start_gateway(&mut self) {
        ...
        self.state.gateway_status = rustyclaw_core::types::GatewayStatus::Connecting;
    ...
    pub async fn stop_gateway(&mut self) {
        ...
        if had_connection || daemon_stopped {
            self.state.gateway_status = rustyclaw_core::types::GatewayStatus::Disconnected;
            if !daemon_stopped {
                self.state
                    .messages
    ...
    pub async fn handle_action(&mut self, action: Action) -> Result<Option<Action>> {
        match action {
            ...
            Action::GatewayResponseDone => {
                ...
                if let Some(buf) = self.streaming_response.take() {
                    ...
                    if let Some(last) = self.state.messages.last_mut() {
                        ...
                        if matches!(last.role, rustyclaw_core::types::MessageRole::Assistant) {
                    ...
                    if !trimmed.is_empty() {
                        if let Some(last) = self.state.messages.last_mut() {
                            if matches!(last.role, rustyclaw_core::types::MessageRole::Assistant) {
                                ...
                                last.role = rustyclaw_core::types::MessageRole::Thinking;
            ...
            Action::GatewayVaultLocked => {
                ...
                self.state.gateway_status = rustyclaw_core::types::GatewayStatus::VaultLocked;
                self.state.messages.push(DisplayMessage::warning(
